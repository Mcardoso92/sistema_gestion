        ...
        public async Task<IActionResult> Anular(AnularCobroVentaVM vm)
        {
            ...
            try
            {
                ...
                if (cajaActual == null)
                {
                    ...
                    ModelState.AddModelError("", "La caja asociada al cobro ya no se encuentra disponible.");
                ...
                decimal saldoDisponibleCaja = await _cajaSaldoService.CalcularSaldoDisponible(cajaActual, usuario.Id);

                if (movimiento!.Importe > saldoDisponibleCaja)
                {
                    await transaccion.RollbackAsync();

                    ModelState.AddModelError("", $"No se puede anular el cobro porque la caja no tiene saldo suficiente. Disponible: {saldoDisponibleCaja:C}.");
